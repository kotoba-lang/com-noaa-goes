(ns noaa-goes.main
  "Kotodama WASM entrypoint for the Noaa_goes clean-room actor (L4) — Clojure port.
  ...
  facts (`noaa_goes.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "noaa_goes")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"noaa_goes.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "noaa_goes-compat" :tier tier :entities entities} 200])
...
(defn start! [] :noaa_goes-compat/ready)
